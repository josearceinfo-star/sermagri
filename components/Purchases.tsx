...
const PurchaseFormModal: React.FC<{
    ...
    onClose: () => void;
}> = ({ suppliers, products, onSave, onClose }) => {
                ...
                <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden space-y-4 pt-6">
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Proveedor</label>
                         <select value={supplierId} onChange={e => setSupplierId(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 bg-white text-gray-900 focus:outline-none focus:ring-green-500 focus:border-green-500" required>
...
export const Purchases: React.FC<PurchasesProps> = ({ purchases, suppliers, products, onAddPurchase }) => {
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      {isModalOpen && <PurchaseFormModal suppliers={suppliers} products={products} onSave={handleSave} onClose={() => setIsModalOpen(false)} />}
    </div>
  );
};
