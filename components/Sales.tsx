...
const SaleDetailModal: React.FC<{ sale: Sale; products: Product[]; onClose: () => void }> = ({ sale, products, onClose }) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-lg">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold">Detalle de Venta - {sale.id}</h2>
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-800">&times;</button>
                </div>
                <p className="mb-4 text-gray-600">Fecha: {new Date(sale.date).toLocaleString('es-CL')}</p>
                <div className="border-t border-b py-2">
                    {sale.items.map(item => (
                        <div key={item.productId} className="flex justify-between items-center py-2">
                            <div>
                                <p className="font-semibold">{findProductName(item.productId)}</p>
                                <p className="text-sm text-gray-600">
                                    {item.quantity} x ${item.price.toLocaleString('es-CL')}
                                </p>
                            </div>
                            <p className="font-semibold">${(item.quantity * item.price).toLocaleString('es-CL')}</p>
                        </div>
                    ))}
                </div>
                <div className="flex justify-end items-center mt-4 pt-4 border-t">
                    <span className="text-lg font-bold">Total:</span>
                    <span className="text-lg font-bold ml-4">${sale.total.toLocaleString('es-CL')}</span>
                </div>
...
export const Sales: React.FC<SalesProps> = ({ sales, products }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {sales.map(sale => (
              <tr key={sale.id} className="bg-white border-b hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <button onClick={() => setSelectedSale(sale)} className="text-blue-600 hover:text-blue-800">Ver Detalles</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {selectedSale && <SaleDetailModal sale={selectedSale} products={products} onClose={() => setSelectedSale(null)} />}
    </div>
  );
};